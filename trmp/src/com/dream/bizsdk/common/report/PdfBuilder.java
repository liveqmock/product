...
public class PdfBuilder implements ReportBuilder {
    ...
    public IBLContext getBLContext() {
        return context;
    }

    public void setBLContext(IBLContext context) {
    ...
    private Object beginNode(Node n) throws DocumentException {
        ...
        if (n != null) {
            ...
            if (nodeName.compareToIgnoreCase("p") == 0) {
                ...
                pe = beginUnderline(n);
            } else if (nodeName.compareToIgnoreCase("br") == 0) {
                pe = beginBr();
    ...
    private Phrase beginLoop(Node n) throws DocumentException {
        ...
        if (nl != null && (count = nl.getLength()) > 0) {
            ...
            if (t.length() > 0) {
                try {
                    loopCount = Integer.parseInt(ExpressionResolver.resolveNumberExp(t, data));
    ...
    private Image beginCImg(Node n) throws DocumentException {
        ...
        String dim1 = e.getAttribute("dim1");
        String dim2 = e.getAttribute("dim2");
        ...
        if (src.length() > 0) {
            try {
                ...
                int i = 0;
                while (true) {
                    String t1 = e.getAttribute("DRMSubjectCond/cCode[" + i + "]");
                    if (t1.length() > 0) {
                        String min = e.getAttribute("DRMSubjectCond/minValue[" + i + "]");
                        String max = e.getAttribute("DRMSubjectCond/maxValue[" + i + "]");
                        d.add("DRMSubjectCode", "cCode", i, t1);
                        if (min.length() > 0) {
                            d.add("DRMSubjectCode", "minValue", i, min);
                        }
                        if (max.length() > 0) {
                            d.add("DRMSubjectCode", "maxValue", i, max);
                        }
                        i++;
                    } else {
                        break;
                ...
                BLResult br = context.execBL("com.dream.app.sys.ImageBLC", "getImageData", d, null);
                BizData imageData = (BizData) br.rd.get("imageData");

                //build the image and save the image to temp file;
                ...
                Image img = Image.getInstance(new URL(file.getAbsolutePath()));
                if (img == null) {
                    return null;
                } else {
                    if (align.length() > 0) {
                ...
                throw new DocumentException(t.getMessage());
            }
        }
        return null;
    }
    ...
    private void beginTrLoop(Node loop, float[] width) throws DocumentException {
        ...
        int resolvedColWidth = 0;
        try {
            String fromStr = ExpressionResolver.resolveNumberExp(((Element) loop).getAttribute("from"), data);
            String toStr = ExpressionResolver.resolveNumberExp(((Element) loop).getAttribute("to"), data);
    ...
    private Chunk beginDi(Node n) throws DocumentException {
        ...
        if (name.length() == 0) {
            ...
            try {
                name = ExpressionResolver.resolveStringExp(name, data);
            } catch (ParseException pe) {
                throw new DocumentException(pe);
            }
            ...
            if (field.length() == 0) {
                if (attr.length() == 0) {
                    return new Chunk(Util.formatValue(data.getString(name), format), pdfContext.getCurrentFont());
                } else {
                    return new Chunk(Util.formatValue(data.getAttr(name, attr), format), pdfContext.getCurrentFont());
                }
            } else {
                if (row.length() == 0) {
                    if (attr.length() == 0) {
                        return new Chunk(Util.formatValue(data.get(name, field, "0"), format), pdfContext.getCurrentFont());
                    } else {
                        return new Chunk(Util.formatValue(data.getAttr(name, field, "0", attr), format), pdfContext.getCurrentFont());
                    }
                } else {
                    try {
                    ...
                    if (attr.length() == 0) {
                        return new Chunk(Util.formatValue(data.get(name, field, row), format), pdfContext.getCurrentFont());
                    } else {
                        return new Chunk(Util.formatValue(data.getAttr(name, field, row, attr), format), pdfContext.getCurrentFont());
                    }
                }
            }
        }
    ...
    private float[] parseTr(Node tr) throws DocumentException {
        ...
        if (nl != null && (tds = nl.getLength()) > 0) {
            for (int i = 0; i < tds; i++) {
                ...
                cell = new Cell(ck);
                try {
                    cell.setColspan(colspan.length() == 0 ? 1 : Integer.parseInt(ExpressionResolver.resolveNumberExp(colspan, data)));
                    cell.setRowspan(rowspan.length() == 0 ? 1 : Integer.parseInt(ExpressionResolver.resolveNumberExp(rowspan, data)));
                } catch (ParseException pe) {
                    throw new DocumentException(pe);
                }
