...
public class Expression {
    ...
    private static Logger log = Logger.getLogger("com.dream.expression");;

    public Expression(String exp, BizData data) throws ParseException {
        parse(exp, data);
        value = exec();
    }

    public Expression(String exp, Hashtable bds) throws ParseException {
        parse(exp, bds);
        value = exec();
    }

    public boolean parse(String exp, BizData data) throws ParseException {
        int index1 = 0;
        int index2 = 0;
        int index = 0;
        int countOP = 0;
        ...
        int start = 0;
        while (i < countOP) {
            RelationalExp expUtil = new RelationalExp(exp.substring(start, indexesOP[i]), data);
        ...
        RelationalExp expUtil = new RelationalExp(exp.substring(start, exp.length()), data);
        sk.push(new Boolean(expUtil.getValue()));
        return true;
    }

    public boolean parse(String exp, Hashtable bds) throws ParseException {
        int index1 = 0;
        int index2 = 0;
        int index = 0;
        int countOP = 0;
        ...
        int start = 0;
        while (i < countOP) {
            RelationalExp expUtil = new RelationalExp(exp.substring(start, indexesOP[i]), bds);
            sk.push(new Boolean(expUtil.getValue()));
            sk.push(new Integer(typeOP[i]));
            if (i < countOP) {
                start = indexesOP[i] + (typeOP[i] == Expression.AND ? ANDOP.length() : OROP.length());
        ...
        RelationalExp expUtil = new RelationalExp(exp.substring(start, exp.length()), bds);
        sk.push(new Boolean(expUtil.getValue()));
        return true;
    }
    ...
    public static boolean valueOf(String exp, BizData data) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("compute boolean expression:" + exp + ", the result value is:" + value);
            }

            return value;
        } catch (Exception e) {
            log.debug("Exception when computing boolean expression:" + exp + "", e);
            return false;
        }
    }
