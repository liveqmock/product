...
public class Zt_RetainedMngBLC extends DBBLC {
	public Zt_RetainedMngBLC(){
		this.entityName="TA_ZT_LC4SHOP";
	}
	//查询购物点留存清款列表	
	public int allLCLshopList(BizData rd, BizData sd) {
		...
		String sql = "select g.id,bv.gwdid,bv.qkzt_lc,bv.gslcxj, sum(case when bv.gwdid=v.gwdid then bcqkje else 0 end) as hkje, t.cmpny_name \n"
				+ " from TA_ZT_BXGW bv,ta_ZT_group g,TA_ZT_LC4SHOP v,ta_shoppoint t \n"
	...
	public int getLcShopByTid(BizData rd,BizData sd){
		String gwdid=rd.getStringByDI("TA_ZT_LC4SHOP","GWDID", 0);
		String tid=rd.getStringByDI("TA_ZT_LC4SHOP","TID", 0);
		String sql="select bx.sfid,bx.csid,bx.gwdid,bx.tid,bx.gslcxj,t.cmpny_name,nvl((select sum(nvl(lc.BCQKJE,0))\n"+  
				" from TA_ZT_LC4SHOP lc\n"+
				" where lc.tid='"+tid+"' and lc.gwdid='"+gwdid+"' ),0) as hkje,bx.gslcxj-nvl((\n"+
				"select sum(nvl(lc.BCQKJE,0))  from TA_ZT_LC4SHOP lc\n"+
				"where lc.tid='"+tid+"' and lc.gwdid='"+gwdid+"' ),0) ye\n"+
				"from  TA_ZT_BXGW bx,TA_ZT_GROUP g,ta_shoppoint t,TA_ZT_LC4SHOP lc\n"+
				...
				"group by  bx.sfid,bx.csid,bx.gslcxj,bx.gwdid,bx.tid,t.cmpny_name";
		try {
...
//添加留存清款	
	public int addLCshop(BizData rd, BizData sd) throws SQLException {
		...
		String tid = rd.getStringByDI("TA_ZT_LC4SHOP", "TID", 0);
		String gwdid=rd.getStringByDI("TA_ZT_LC4SHOP", "GWDID", 0);
		String ye = rd.getStringByDI("TA_ZT_LC4SHOP", "YE", 0);
		try {
			coreDAO.beginTrasct(conn);

			if ("0".equals(ye)) {
				int id = queryMaxIDByPara("TA_ZT_LC4SHOP", "ID", null);
				rd.add("TA_ZT_LC4SHOP", "ID", id);
				rd.add("TA_ZT_LC4SHOP", "qkrq", rd.sdfDate
						.format(new Date()));
				rd.add("TA_ZT_LC4SHOP", "fkrq", rd.sdfDate
						.format(new Date()));
				rd.add("TA_ZT_LC4SHOP", "czrq", rd.sdfDate
						.format(new Date()));
				BizData data1 = new BizData();
				data1.addAttr("TA_ZT_BXGW", "TID", 0, "oldValue", tid);
				data1.addAttr("TA_ZT_BXGW", "GWDID", 0, "oldValue", gwdid);
				data1.add("TA_ZT_BXGW", "QKZT_LC", "Y");

						
				coreDAO.insert("TA_ZT_LC4SHOP", rd, conn);
				coreDAO.update("TA_ZT_BXGW", data1, conn);
			}
...
//查看留存清款列表	
	public int queryLCshoplist(BizData rd,BizData sd){
		...
		String sql="select a.id,a.tid,nvl(a.bcqkje,0) as bcqkje ,a.ye,a.czRQ, b.gslcxj,d.cmpny_name"+
		   " from TA_ZT_LC4SHOP a,TA_ZT_BXGW b,ta_shoppoint d"+
		    " where a.tid=b.tid and b.gwdid=d.shop_point_id"+
		    " and a.gwdid='"+gwdid+"' and d.shop_point_id='"+gwdid+"' and a.tid='"+tid+"'"+
