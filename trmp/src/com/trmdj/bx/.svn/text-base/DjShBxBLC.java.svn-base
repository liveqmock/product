 ...
 * @deprecated 
 */
public class DjShBxBLC extends DBBLC {
	public DjShBxBLC(){
	...
	public int querySpBxInfo(BizData rd,BizData sd) {
		...
		sql.append("select f.*,g.xlmc,g.begin_date,g.end_date,n.nodename,f.DEFINITIONID\n")
			...
			.append("and f.isgroup='D' and (f.nextroleid in ("+roles+") or f.creater = "+sd.getString("userno")+") and n.DEFINITIONID='djbxsh' and g.orgid="+sd.getString("orgid")+"\n");
	...
	public void querySpyj(BizData rd,BizData sd){
		String spyj="select t.spyj,u.username from ta_dj_tspb t,drmuser u where t.spr=u.userno and t.mklb='2' and t.tid='"+rd.getString("TA_DJ_GROUP","ID",0)+"' and t.orgid= '"+ sd.getString("orgid")+"'";
		try {
	...
	public int authorizeBx(BizData rd,BizData sd) {
		...
		try{
			...
			data.add("TA_FLOW", "DEFINITIONID", "djbxsh");
			...
			if("N".equals(yjzt)){
				queryCreateRole(rd, groupId, sd.getString("orgid"));
			...
			//无下级节点，流程将结束
				...
				if(null == nextNodeID || "".equals(nextNodeID)){
					...
					data1.addAttr("TA_DJ_GROUP", "ID", 0, "oldValue", groupId);
					data1.addAttr("TA_DJ_GROUP", "orgid", 0, "oldValue", sd.getString("orgid"));
					data1.add("TA_DJ_GROUP", "STATE", "6");
					coreDAO.update("TA_DJ_GROUP", data1, conn);
				}
			}
	...
	public int submitBx(BizData rd,BizData sd) {
		...
		try {
			...
			data.addAttr("TA_DJ_GROUP", "ID", "0", "oldVAlue", groupID);
			data.addAttr("TA_DJ_GROUP", "orgid", "0", "oldVAlue", sd.getString("orgid"));
			data.add("TA_DJ_GROUP", "STATE", "5");
			coreDAO.update("TA_DJ_GROUP", data, conn);
			
			updateTDBXZJXXB(rd, conn, groupID, sd.getString("orgid"));
			coreDAO.commitTrasct(conn);
		} catch (SQLException e) {
			try {
				coreDAO.rollbackTrasct(conn);
	...
	public void updateTDBXZJXXB(BizData rd, Connection conn, String groupID, String orgid){
		BizData data1 = new BizData();
		data1.add("TA_TDBXZJXXB", "TID", groupID);
		data1.add("TA_TDBXZJXXB", "orgid", orgid);
		try {
			coreDAO.delete(data1,conn);
			rd.add("TA_TDBXZJXXB", "orgid", orgid);
	...
	private void updateFLowInfo(Connection conn, String groupID, String currentRole, String state,
		...
		data.add("TA_FLOW", "ISGROUP", "D");data.add("TA_FLOW", "ISGROUP", "D");
	...
	private void queryNextNodeInfoByGroupID(BizData rd, String groupId) throws SQLException {
		...
		sql.append("select a.*,n2.nodeid nextid,n2.nodename nextname,n2.roleid nextroleid\n")
			...
			.append("and f.tid='"+groupId+"' and f.definitionid='djbxsh' and f.isgroup='D'\n")
			.append(") a,ta_flownode n2\n")
			.append("where a.nodeid=n2.pid(+)");
	 ...
	 * @throws SQLException 
	 */
	private void queryCreateRole(BizData rd, String groupID, String orgid) throws SQLException {
		
		StringBuffer sql = new StringBuffer();
		sql.append("select n.roleid,n.nodeid\n")
			.append("from ta_flow f, ta_flownode n\n")
			.append("where f.definitionid=n.definitionid and f.orgid=n.orgid and f.orgid=").append(orgid).append("\n")
			.append("and n.pid=-1 and f.definitionid='djbxsh'\n")
			.append("and f.isgroup='D'")
	...
	*/
	public int submitBxBJ(BizData rd,BizData sd){
		...
		String planBjInfo="select j.* \n" +
						" from ta_tdjdxxzjb j where j.tid='"+groupID+"' and j.orgid='"+orgid+"'" ;
		String bxBjInfo="select b.* \n" +
				" from ta_tdbxzjxxb b where b.tid ='"+groupID+"' and b.orgid='"+orgid+"'";
		try {
			coreDAO.executeQuery(bxBjInfo, "bxBjInfo", rd, conn);
			coreDAO.executeQuery(planBjInfo, "planBjInfo", rd, conn);
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			try{
