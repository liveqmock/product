...
<script language="JavaScript">    
	...
	*/    
	...
	function create() {
		...
		var frameid = parent.document.getElementById("empRight"); 
		frameid.src = "initEmpAdd.?dispMode=N&hrorganization/orgid="+orgID;
	}
	function update()    
	{    
...
<div class="dtree">
	...
	%>
		d.add(<%=orgID %>
		,<%=parentOrgID %>
		,'<a href="<%=url%>HREMPLOYEE/ORGID=<%=orgID%>&HREMPLOYEE@pageNO=1&HREMPLOYEE@pageSize=10" target="empRight"><%=orgName%></a>','','','');
	<%
	...
	d.add(<%=orgIDSec %>
	,<%=orgParentOrgIDSec %>
	,'<a href="<%=url%>HREMPLOYEE/ORGID=<%=orgIDSec%>&HREMPLOYEE@pageNO=1&HREMPLOYEE@pageSize=10" oncontextmenu=showMenu("<%=orgIDSec%>") target="empRight"><%=orgNameSec%></a>','','','');
	<%
	...
	d.add(<%=orgIDLast %>
	,<%=orgParentOrgIDLast %>
	,'<a href="<%=url%>HREMPLOYEE/ORGID=<%=orgIDLast%>&HREMPLOYEE@pageNO=1&HREMPLOYEE@pageSize=10" oncontextmenu=showMenu("<%=orgIDLast%>") target="empRight"><%=orgNameLast%></a>','','','');
	<%				
					}
				}
			}
