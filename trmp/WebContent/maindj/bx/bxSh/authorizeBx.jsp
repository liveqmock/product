		...
		function addTrRow(mdName,csName){
			...
			if('plusMd' == mdName){
				...
				$("#plusMd").append('<tr class="plusRow" style="display: block;">' +
				  ...
				  '<td>'+
					'导游：<input type="text" name="TA_DJ_BXJIADIAN/DYTC['+trRows+']" onkeydown="checkNumX()" title="导游提成/单位：元" onkeyup="cSn(\'plus\');" onchange="cSnChange(\'plus\')" class="input40 plusdytc"/><p>'+
					'司机：<input type="text" name="TA_DJ_BXJIADIAN/SJTC['+trRows+']" onkeydown="checkNumX()" title="司机提成/单位：元" onkeyup="cSn(\'plus\');" onchange="cSnChange(\'plus\')" class="input40 plussjtc"/><p>'+
					'全陪：<input type="text" name="TA_DJ_BXJIADIAN/QPTC['+trRows+']" onkeydown="checkNumX()" title="全陪提成/单位：元" onkeyup="cSn(\'plus\');" onchange="cSnChange(\'plus\')" class="input40 plusqptc"/><p>'+
					'公司：<input type="text" name="TA_DJ_BXJIADIAN/GSTC['+trRows+']" onkeydown="checkNumX()" title="公司提成/单位：元" onkeyup="cSn(\'plus\');" onchange="cSnChange(\'plus\')" class="input40 plusTemp4"/><p>'+
				  '</td>'+
...
<tr>
	<td>
		<ul>
			<li class="ckeditorli">
				<a class="a" href="###" style="color:#006666;" onclick="xcmx1('7','shopMd','djAjaxShopPlanInfo.?groupId=<%=groupId%>');">购物明细&nbsp;<span>【<font color="red"><var id="shopRow"><%=rd.getStringByDI("JHGW","SUM",0) %></var></font>条信息】</span><span>【购物进店人数：<font color="red"><var id="shopTemp4"><%=rd.getStringByDI("JHGW","JDRS",0) %> </var></font>人】</span><span>【购物消费金额：<font color="red"><var id="shopTemp3"><%=rd.getStringByDI("JHGW","XFE",0) %> </var></font>元】</span><span>【购物应交公司：<font color="red"><var id="shopTemp5"><%=rd.getStringByDI("JHGW","YJGS",0) %> </var></font>元】</span><span>【购物人头费：<font color="red"><var id="shopTemp1"><%=rd.getStringByDI("JHGW","RTF",0) %> </var></font>元】</span><span>【购物公司留存：<font color="red"><var id="shopTemp2"><%=rd.getStringByDI("JHGW","GSLC",0) %> </var></font>元】</span></a>
				<ul>
					<li>
						<div id="list-table">
				  		  <table class="datas">
  ...
  <td>
	<ul>
	  <li class="ckeditorli" style="background: url(<%=request.getContextPath()%>/images/header-bg.gif) repeat">
		<a class="a" href="###" style="color:#006666;" onclick="xcmx1('8','plusMd','djAjaxPlusPlanInfo.?groupId=<%=groupId%>');">加点明细&nbsp;<span>【<font color="red"><var id="plusRow"><%=rd.getStringByDI("JHJIAD","SUM",0) %></var></font>条信息】</span><span>【加点成本签单：<font color="red"><var id="plusCbqd"><%=rd.getStringByDI("JHJIAD","CBQD",0) %> </var></font>元】</span><span>【加点成本现付：<font color="red"><var id="plusCbxf"><%=rd.getStringByDI("JHJIAD","CBXF",0) %> </var></font>元】</span><span>【加点预估成本：<font color="red"><var id="plusYgcb"><%=rd.getStringByDI("JHJIAD","CBJE",0) %></var></font>元】</span><span>【加点人数合计：<font color="red"><var id="plusSnTemp2"><%=rd.getStringByDI("JHJIAD","RS",0) %></var></font>人】</span><span>【加点利润合计：<font color="red"><var id="plusTemp2"><%=rd.getStringByDI("JHJIAD","LR",0) %></var></font>元】</span><span><var>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</var>【加点应交公司：<font color="red"><var id="plusTemp3"><%=rd.getStringByDI("JHJIAD","YJGS",0) %></var></font>元】</span></a>
		<ul>
	  	  <li>
	  		<div id="list-table">
	  		  <table id="tbNone" class="datas">
...
function autoCount(){	
	xfhj=/*现付合计*/
	isN(parseFloat($("#ticketCbxf").text()))+isN(parseFloat($("#carCbxf").text()))+isN(parseFloat($("#hotelCbxf").text()))+
	...
	cbzc=/*成本支出=酒店合计+餐厅合计+票务合计+车辆合计+景点合计+地接合计+其他合计+导服费*/
	isN(parseFloat($("#ticketYgcb").text()))+isN(parseFloat($("#carYgcb").text()))+isN(parseFloat($("#hotelYgcb").text()))+
	...
	yjcwxjhj=/*应交财务现金合计 = 导游领款 - 现付合计 + 购物应交公司 + 加点应交公司*/
	isN(parseFloat($("#guideTemp1").text()))-xfhj+isN(parseFloat($("#shopTemp5").text()))+isN(parseFloat($("#plusTemp3").text()));
	
	gwlrhj=/*购物利润合计=人头费+留存+应交现金*/
		isN(parseFloat($("#shopTemp1").text()))+isN(parseFloat($("#shopTemp2").text()))+isN(parseFloat($("#shopTemp5").text()));
	
	jdlrhj=/*加点利润合计=人头费+净利*/
		isN(parseFloat($("#plusTemp2").text()));
	
	tdlr=/*团队利润 = 营收 - 合计  + 购物利润合计  + 加点利润合计 */
		isN(parseFloat($("#groupTkzj").val()))-cbzc+gwlrhj+jdlrhj;
	zmml =/*账面毛利=营收-成本合计*/
		isN(parseFloat($("#groupTkzj").val()))-cbzc;
	qdhj=/*签单合计=成本支出-现付合计-导服费*/
		cbzc-xfhj-isN(parseFloat($("#guideTemp2").text()));
...
function bxSubmit(){
	document.bxShform.action="djAuthorizeBx.?STATE="+$("#STATE").val()+"&TID=<%=groupId %>";
  ...
  <table class="datas">
	...
	<tr>
		<td  colspan="2"><span>审批意见</span>&nbsp;&nbsp;&nbsp;&nbsp;
			<span class="showPointer" style=" text-decoration:underline; color:red;font-weight:100" onclick="TB_show('查看数据对比信息','<%=request.getContextPath()%>/maindj/bx/bxSh/djBxBJ.?TID=<%=groupId %>*TB_iframe=true&height=400&width=500'),''">数据对比&nbsp;</span>
				<img src="<%=request.getContextPath()%>/images/Contrast.gif" height="16" width="16" class="showPointer" onclick="TB_show('查看数据对比信息','<%=request.getContextPath()%>/maindj/bx/bxSh/djBxBJ.?TID=<%=groupId %>*TB_iframe=true&height=400&width=500'),''"/>
		</td>
	</tr>
	<tr>
		<td  colspan="2">
	...
	<tr>
		<td colspan="2" align="center">
		  <br>
			<input type="button" value="提交" onclick="bxSubmit();"/>
