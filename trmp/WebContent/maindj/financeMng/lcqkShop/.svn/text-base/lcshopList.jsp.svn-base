...
function findByLike(){
	document.dj_guide_form.action="LCshopList.?&pageSize=10&pageNO=1";
	document.dj_guide_form.submit();
}
function addLC(gwdid,tid)
{
	window.open("trm/maindj/financeMng/getLcByTid.?TA_DJ_LC4SHOP/GWDID="+gwdid+"&TA_DJ_LC4SHOP/TID="+tid,"newwindow","height=400,width=400,top=180,left=500,toolbar=no,menubar=no,scrollbars=no,resizablr=no,locatiog=no,status=no");
}
function viewLC(gwdid,tid)
{
	window.open("trm/maindj/financeMng/queryLClist.?TA_DJ_LC4SHOP/GWDID="+gwdid+"&TA_DJ_LC4SHOP/TID="+tid,"newwindow2","height=400,width=600,top=180,left=400,toolbar=no,menubar=no,scrollbars=yes,resizablr=no,locatiog=no,status=no");
}
</script>
</head>
	...
	<table class="datas" >
		...
		<tr>
            <td><%=tid %></td>
	  		...
	  		<td><%=hkje %>元</td>	  
	  		<td><%if("N".equals(qkzt)) {%>
	  			<a href="###" onclick="addLC('<%=gwdid %>','<%=tid %>');"  >
           		<img alt="编辑" src="<%=request.getContextPath()%>/images/edit.gif">[结算留存款]&nbsp;</a>
           		<% } %>
           		<a href="###" onclick="viewLC('<%=gwdid %>','<%=tid %>');"  >
...
<script type="text/javascript">
	...
	var linkage = new Linkage("dataSrc0", "<%=request.getContextPath()%>/main/data/Shopd.xml");
	linkage.init();
