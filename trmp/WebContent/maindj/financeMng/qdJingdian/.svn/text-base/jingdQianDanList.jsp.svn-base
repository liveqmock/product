<%@ page language="java" contentType="text/html; charset=GBK"
 ...
 <%
	...
	if(rd.getTableRowsCount("queryAlljingdList")>0){
		...
		totalPage=(Integer)rd.getAttr("queryAlljingdList", "pagesCount");
		rowsCount = (Integer)rd.getAttr("queryAlljingdList", "rowsCount");
	}
%>   
...
<div id="lable"><span>景点签单列表</span></div>
...
<div id="list-table">
	<table class="datas" >
		...
		<%
		int rows = rd.getTableRowsCount("queryAlljingdList");
		...
		<tr>
            <td><%=tid %></td>
	  		...
	  		<td><%=hkje %>元</td>	  
	  		<td>
	  			<a href="###" onclick="javascript:GB_myShow('','/trm/maindj/financeMng/getBXJingDByTid.?TA_DJ_QDQKJL4JD/JDID=<%=id %>&TA_DJ_QDQKJL4JD/TID=<%=tid %>','400','400','')" >
           		<img alt="编辑" src="<%=request.getContextPath()%>/images/edit.gif">[结算签单]&nbsp;</a>
           		<a href="###" onclick="javascript:GB_myShow('','/trm/maindj/financeMng/queryJingDlist.?TA_DJ_QDQKJL4JD/JDID=<%=id %>&TA_DJ_QDQKJL4JD/TID=<%=tid %>','400','600','')" >
...
<div id="page">
	<%String url=request.getContextPath()+"/maindj/financeMng/AllJingDList.?pageSize=10&pageNO=";%>
	...
	<span class="pagination-sum">
					...
					共<%=rd.getAttr("queryAlljingdList","rowsCount")==null?"0":rd.getAttr("queryAlljingdList","rowsCount") %> 条记录，
					每页 <%=pageSize%>条
	</span>
