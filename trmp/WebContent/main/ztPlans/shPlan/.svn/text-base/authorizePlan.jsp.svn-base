...
<script type="text/javascript">
	...
	function p_editSpPlan(){
		...
		document.p_shplan_form.action="ztaddShPlan.?TID="+groupId+"&pageNO=1&pageSize=10&flowid=ztywsh";
...
<table id="groupDiv" class="datas">
	...
	<%
		...
		String yskzj=rd.getStringByDI("selectYsk","ddysk",0);
	%>
<table class="datas" style="margin-top: 3px;">
	<tr>
		<td ><span>酒店费用合计</span> <span class="showPointer" title="显示酒店费用明细" onclick="return GB_myShow('酒店计划','<%=request.getContextPath()%>/main/ztPlans/hotelPlan/ztSelectHotelPlanInfo.?TA_ZT_JHHOTEL/ID=<%=rd.getStringByDI("TA_ZT_GROUPs","ID",0) %>&TA_TDJDXXZJB_ZT/TID=<%=rd.getStringByDI("TA_ZT_GROUPs","ID",0) %>&TID=<%=rd.getStringByDI("TA_ZT_GROUPs","ID",0) %>','800','850','')">显示明细</span></td>
	</tr>
	<tr>
		<td align="right">
  ...
  <tr>
	<td align="right">
	  ...
	  <%
	  	...
	  	float yskje=!"".equals(yskzj)?Float.parseFloat(yskzj):0;
	  	float jqzj = zs+ct+jd+pw+cl+dj+dffzj;
	  %>
	  <%=jqzj %>/元
	   <font color="red">营收：</font>
	   <%=yskje %>/元
...
<table class="datas">
	...
	<tr>
		...
		<td>审批意见:
			...
			<input type="hidden" name="TA_DJ_TSPB/MKLB" value="1">
			<input type="hidden" name="TA_DJ_TSPB/TID" value="<%=rd.getStringByDI("TA_ZT_GROUPs","ID",0)%>">
		</td>
	</tr>
</table>
</div>
...
<div align="center" id="last-sub">
	<input type="button" id="button" value="提交" onclick="p_editSpPlan();"/>
