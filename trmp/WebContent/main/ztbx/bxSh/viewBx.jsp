...
<%
String viewState = rd.getString("view");
...
<table class="datas" >
		...
		<table class="datas">
			<tr>
				<td><span>导游费用信息</span><span class="showPointer" onclick="return GB_myShow('导游费用报销','<%=request.getContextPath() %>/maindj/bx/bxGuide/djInitGuideRbt.?TA_ZT_BXDY/TID=<%=ID %>&TA_TDBXZJXXB_ZT/TID=<%=ID %>&flag=view','800','650','')">&nbsp;&nbsp;查看报账明细</span></td>
			</tr>
			<tr>
				<td align="right">
					导游费用合计:导游领款合计:<font color="red"><%=dylk %></font>元    导服费合计:<font color="red"><%=dff %></font>元 导游交通费合计:<font color="red"><%=dyjtf %></font>元&nbsp;&nbsp;&nbsp;&nbsp;
				</td>
			</tr>
	...
	<%
		...
		<table class="datas">
			<tr>
				...
				<td><span>酒店报帐信息</span><span class="showPointer" onclick="return GB_myShow('酒店费用报销','<%=request.getContextPath()%>/maindj/bx/bxHotel/initDjHotelBx.?TA_ZT_GROUP/ID=<%=ID%>&TA_ZT_BXHOTEL/TID=<%=ID %>&TID=<%=ID %>&flag=view','800','750','')">&nbsp;&nbsp;查看报账明细</span></td>
	...
	<div id="dinnerDiv">
		...
		<table class="datas">
			...
			<td><span>餐厅报帐信息</span><span class="showPointer" onclick="return GB_myShow('餐厅费用报销','<%=request.getContextPath()%>/maindj/bx/bxRestaurant/initDjDinnerBx.?TA_ZT_GROUP/ID=<%=ID%>&TA_ZT_BXCT/TID=<%=ID %>&TID=<%=ID %>&flag=view','800','650','')">&nbsp;&nbsp;查看报账明细</span></td>
	...
	<%
		...
		<table class="datas">
			...
			<td><span>票务报帐信息</span><span class="showPointer"  onclick="return GB_myShow('票务报销','<%=request.getContextPath()%>/maindj/bx/bxTicket/djInitTicketBx.?TA_ZT_BXPW/TID=<%=ID %>&TA_ZT_JHPW/TID=<%=ID %>&dmsm/JTGJ=2&flag=view','800','850','')">&nbsp;&nbsp;查看报账明细</span></td>
	...
	<%
		...
		<table class="datas">
			...
			<td><span>车辆报销信息</span><span class="showPointer" onclick="return GB_myShow('车辆报销','<%=request.getContextPath()%>/maindj/bx/bxCar/djInitVehBx.?TA_ZT_JHCL/TID=<%=ID %>&TA_ZT_BXCL/TID=<%=ID %>&flag=view','800','850','')">&nbsp;&nbsp;查看报账明细</span></td>
	...
	<table class="datas">
			...
			<td><span>地接报销信息</span><span class="showPointer"  onclick="return GB_myShow('地接社报销','<%=request.getContextPath()%>/maindj/bx/bxGroundTrave/djInitdjsInfo.?TA_ZT_BXDJ/TID=<%=ID %>&flag=view','800','600')">&nbsp;&nbsp;查看报账明细</span></td>
	...
	<%
		...
		<table class="datas">
			...
			<td><span>景点报账信息</span><span class="showPointer" onclick="return GB_myShow('查看景点报账信息','<%=request.getContextPath() %>/maindj/businessPlan/plan/sceneryPlan/djInitSceneryRbt.?TA_ZT_BXJD/TID=<%=ID %>&flag=view','800','700','')">&nbsp;&nbsp;查看报账明细</span></td>
	...
	<%
		...
		<table class="datas">
			...
			<td><span>购物报账信息</span><span class="showPointer"  onclick="return GB_myShow('查看购物报账信息','<%=request.getContextPath() %>/maindj/businessPlan/plan/sceneryPlan/djInitShopRbt.?TCCY=18&LCBL=21&TA_ZT_BXGW/TID=<%=ID %>&TA_TDBXZJXXB_ZT/TID=<%=ID %>&flag=view&qprszt=<%=QPRSZT %>&CRRS=<%=CRRS %>','800','700','')">&nbsp;&nbsp;查看报账明细</span></td>
	...
	<%
		...
		<table class="datas">
			...
			<td><span>加点报账信息</span><span class="showPointer" onclick="return GB_myShow('查看加点报账信息','<%=request.getContextPath() %>/maindj/businessPlan/plan/sceneryPlan/djInitPlusRbt.?TCCY=18&LCBL=21&TA_ZT_BXJIADIAN/TID=<%=ID %>&TA_TDBXZJXXB_ZT/TID=<%=ID %>&flag=view&qprszt=<%=QPRSZT %>','800','700','')">&nbsp;&nbsp;查看报账明细</span></td>
	...
	<div id="SumPrice">
		<table class="datas">
			...
			<%
				...
				Float TDLR = yskzjs - jdhjs -cthjs - pwhjs - clhjs -jindhjs - djhjs  + gwlrhjs + jdlrhjs +jdrtfhjs;
			%>
			<tr>
