...
%>	
  <tr>
	<td width="34%"><%=priceName %>
	  <input type="hidden" name="TA_ZT_BXJDJG<%=JDID %>/JGMCID[<%=i %>]" value="<%=priceNMC %>"/>
	  <input type="hidden" name="TA_ZT_BXJDJG<%=JDID %>/JGMC[<%=i %>]" value="<%=priceName %>" />
	</td>
	<td width="33%"><%=price %>
	  <input type="hidden" name="TA_ZT_BXJDJG<%=JDID %>/JG[<%=i %>]" value="<%=price %>" class="sc_price"/>
	</td>
	<td width="33%">
	  <input type="text" name="TA_ZT_BXJDJG<%=JDID %>/RS[<%=i %>]" class="smallerInput sc_num" value="0"  onkeydown="checkNum()" onchange="sumPrice('scenery<%=JDID%>_tab')"/>
	</td>
  </tr>
<%
}%>
<tr>
  <td align="right" colspan="7">
	<font color="red">签单小计金额：</font>
	  <input type="text" name="QDXJ<%=JDID %>" value="0" type="text" readonly="readonly" class="smallerInput qdxj" onchange="sumPrice('scenery<%=JDID%>_tab')" onkeydown="checkNum1(this)"/>/元&nbsp;&nbsp;&nbsp;
	<font color="red">现付小计金额：</font>
	  <input type="text" name="XFXJ<%=JDID %>" value="0" type="text" readonly="readonly" class="smallerInput xfxj"/>/元&nbsp;&nbsp;&nbsp;
	<font color="red">共计：</font>
			<input  name="HJ<%=JDID %>" value="0" type="text"  readonly="readonly" class="smallerInput gj"/>/元
  </td>
</tr>	
</table>
