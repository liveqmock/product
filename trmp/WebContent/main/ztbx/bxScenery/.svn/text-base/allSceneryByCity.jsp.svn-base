...
%>
 <table class="datas select-scenery" width="95%" id="itemTab<%=JDID %>">
	<tr>
	  ...
	  <td width="20%">
	  	<input name="dwj<%=JDID %>" type="radio" value="1" checked="checked" id="dwj"/>永久
	  	<input name="dwj<%=JDID %>" type="radio" value="2" id="dwj"/>淡季价格
	  	<input name="dwj<%=JDID %>" type="radio" value="3" id="dwj"/>旺季价格
	  </td>
	  <td width="10%">
	  	<input type="hidden" name="JDID<%=CSID %>/JDID[<%=l %>]" value="<%=JDID %>" />
	    <input type="checkbox" name="SFXZ<%=JDID %>" onclick="queryAllPriceByScenery(this.value,'itemTab<%=JDID %>','dwj<%=JDID %>','<%=JDID %>')" 
