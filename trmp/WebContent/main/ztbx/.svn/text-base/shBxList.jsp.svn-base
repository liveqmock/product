...
function queryGroupList(){
	document.djListGroup.action.name="ztQueryGroupList.?pageNO=1&pageSize=10";
...
function showDjGroupID(ID){
	GB_myShow('审核','ztInitAllSHPlan.?TA_ZT_GROUP/ID='+ID+'&TA_TDJDXXZJB/TID='+ID);
}
</script>
...
<div id="list-table">
	<table class="datas" >
		...
		<tr>
			...
			%>
			<a href="<%=request.getContextPath()%>/main/ztPlans/shPlan/ztInitAllSHBx.?TA_ZT_GROUP/ID=<%=ID %>&TA_ZT_BXHOTEL/TID=<%=ID %>&TA_ZT_BXCT/TID=<%=ID %>&TA_ZT_BXJD/TID=<%=ID %>&TA_ZT_BXDJ/TID=<%=ID %>&TA_ZT_BXDY/TID=<%=ID %>&TA_ZT_BXCL/TID=<%=ID%>&TA_ZT_BXPW/TID=<%=ID %>&TA_ZT_BXGW/TID=<%=ID %>&TA_ZT_BXJIADIAN/TID=<%=ID %>&TA_TDBXZJXXB_ZT/TID=<%=ID %>&TA_DJ_TSPB/TID=<%=ID %>&action=view&flag=view&pageNO=1&pageSize=10"><img alt="查看" src="<%=request.getContextPath()%>/images/edit.gif">&nbsp;[查看]</a>
			<%
			}
			if(creater.equals(sd.getString("userno")) && isBack.equals("Y")){
			%>
				<a href="<%=request.getContextPath()%>/maindj/bx/ztrbtGuideListByUser.?TA_ZT_GROUP/ID=<%=ID %>&TA_ZT_BXHOTEL/TID=<%=ID %>&TA_ZT_BXCT/TID=<%=ID %>&TA_ZT_BXJD/TID=<%=ID %>&TA_ZT_BXDJ/TID=<%=ID %>&TA_ZT_BXDY/TID=<%=ID %>&TA_ZT_BXCL/TID=<%=ID%>&TA_ZT_BXPW/TID=<%=ID %>&TA_ZT_BXGW/TID=<%=ID %>&TA_ZT_BXJIADIAN/TID=<%=ID %>&TA_TDBXZJXXB_ZT/TID=<%=ID %>&TA_DJ_TSPB/TID=<%=ID %>&isCheck=Y"><img alt="修改" src="<%=request.getContextPath()%>/images/edit.gif">&nbsp;[修改]</a>
			<%	
			}
			if(isTrue == true && !"end".equals(flowState)){
			%>
			<a href="<%=request.getContextPath()%>/main/ztPlans/shPlan/ztInitAllSHBx.?TA_ZT_GROUP/ID=<%=ID %>&TA_ZT_BXHOTEL/TID=<%=ID %>&
			TA_ZT_BXCT/TID=<%=ID %>&TA_ZT_BXJD/TID=<%=ID %>&TA_ZT_BXDJ/TID=<%=ID %>&TA_ZT_BXDY/TID=<%=ID %>&TA_ZT_BXCL/TID=<%=ID%>&
			TA_ZT_BXPW/TID=<%=ID %>&TA_ZT_BXGW/TID=<%=ID %>&TA_ZT_BXJIADIAN/TID=<%=ID %>&TA_TDBXZJXXB_ZT/TID=<%=ID %>&TA_DJ_TSPB/TID=<%=ID %>&
			action=authorize&pageNO=1&pageSize=10"><img alt="审核" src="<%=request.getContextPath()%>/images/edit.gif">&nbsp;[审核]</a>
			<%
			}%>
			</td>
		</tr>
...
<div id="page">
	<%String url=request.getContextPath()+"/main/ztPlans/ztQueryShBxList.?pageSize=10&pageNO=";%>
