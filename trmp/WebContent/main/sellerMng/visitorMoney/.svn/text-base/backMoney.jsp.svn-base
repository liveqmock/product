...
<script type="text/javascript">
	function doSub(){
		...
		if(parseInt(alreadyOld) < parseInt(thisTime)) {
			alert("退款金额不能大于已收账款！");
			return false;
		}
...
<body>
<form action="updateMoney.?action=back" method="post" name="myForm">
<input type="hidden" name="tid" value="<%=rd.getString("tid") %>"/>
...
<div class="add-table"> 
  <table class="datas">
	<tr class="first-tr">
	  <td colspan="6"><span>收款确认 </span></td>
	</tr>
	<tr>
	  ...
	  <td width="10%" align="right">本次退款：</td>
	  <td width="15%"><input type="text" name="TA_ZT_YKORDER/TKJE" class="smallInput" id="thisTime" value="<%=rd.getStringByDI("rsMoneyInit","yi_sk",0) %>"/>元</td>
	  <td width="10%" align="right">手续费：</td>
	  <td width="15%"><input type="text" class="smallInput" name="TA_ZT_YKORDER/TKSXF" value="<%=rd.getStringByDI("rsMoneyInit","TKSXF",0) %>"/>元</td>
	</tr>
  </table>
</div>
...
<%
String orderConfirm = rd.getStringByDI("rsMoneyInit","DD_CONFIRM",0);
%>
	<input type="checkbox" name="DD_CONFIRM" value="2" <%if("2".equals(orderConfirm)){ %>checked="checked" <%} %>/>确认取消订单&nbsp;&nbsp;
