...
String gID = rd.getStringByDI("rsDateOfLinePlan","g_id",0);
...
for(int i=0;i<rows;i++){
	...
	String state = rd.getStringByDI("rsDateOfLinePlan","state",i);
	String stateNm = rd.getStringByDI("rsDateOfLinePlan","state",i);
	if("1".equals(state))
		state = "正常";
	else
		state = "已封团";
%>
    arr[<%=i%>] = { year: "<%=planDate %>", total: "<%=maxPersonCount %>", left: "<%=spare %>" , state: "<%=state %>", stateNm: "<%=stateNm %>"};
<%
}
%>
    for (var j = 0, len = arr.length; j < len; j++) {
       ...
       if (this.Year == arr[j]['year'].split("-")[0] && this.Month == arr[j]['year'].split("-")[1]) {
         ...
         var url = "";
         if(state == '1')//正常
             url = "<%=request.getContextPath()%>/main/group/editGroupInit.?TA_ZT_GROUP/id=<%=groupID%>&TA_ZT_LINEMNG/LINE_ID=<%=lineid%>&TA_ZT_GPRICE/G_ID=<%=groupID%>&dmsm/JTGJ=2&dmsm/jglx=4&TA_ZT_GROUP/begin_date="+this.Year+"-"+arr[j]['year'].split("-")[1]+"-"+arr[j]['year'].split("-")[2]+"&TA_ZT_LINEDETAI4G/TID=<%=groupID %>&TA_ZT_GATHER/LINE_ID=<%=lineid%>&TA_ZT_GATHER_HIS/GATHER_ID=&";
         else //已封团
             url = "initNewGroup.?lineID=<%=lineid %>&dmsm/JTGJ=2&dmsm/jglx=4&";  
         ...
         "<a href='"+url+"bDate="+this.Year+"-"+arr[j]['year'].split("-")[1]+"-"+arr[j]['year'].split("-")[2]+"&TA_ZT_LINEDETAI4G@orderBy=rq');\" >" + 
         "</br>" + "总数：" + arr[j]['total'] + "</br>" + "剩余：" + arr[j]['left'] + "</br>"+ "团状态："+arr[j]['state']+"</a></br>";
       }
     }
  }
});
