...
function doSUB(){
	...
	if(bTime != '' && eTime != '')
		window.location.href="http://192.168.0.2:8080/reports/preview?__report=report/Profitability/ShoppingByT.rptdesign&groupBtime="+bTime+"&groupEtime="+eTime;
	if(groupID != ''){
		window.location.href="http://192.168.0.2:8080/reports/preview?__report=report/Profitability/ShoppingByG.rptdesign&groupID="+groupID;
	}
}
</script>
</head>
...
<table border="0" cellpadding="0" cellspacing="0"   class="top_table" width="100%" >
  <tr  class="toptable_pingpu">
    <td width="18%" height="35px"><strong><img src="../images/ico2.jpg">&nbsp;购物分析</strong></td>
    <td width="9%" ></td>
    <td width="12%"></td>
    <td width="12%"></td>
  </tr>
