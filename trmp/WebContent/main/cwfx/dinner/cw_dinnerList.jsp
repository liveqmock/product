...
<%
	...
	int totalPage =1;
	int rowsCount = 1;
	if(rd.getTableRowsCount("dinnerYFZK")>0){
	 ...
	 pageSize=Integer.parseInt((String)rd.getAttr("TA_GROUP_BASE", "pageSize"));
		totalPage=(Integer)rd.getAttr("dinnerYFZK", "pagesCount");
		rowsCount = (Integer)rd.getAttr("dinnerYFZK", "rowsCount");
	}
%>      
...
<script type="text/javascript">
	function findByLike(){
		document.dj_form.action="dinner_yfzk.?TA_GROUP_BASE@pageNO=1&TA_GROUP_BASE@pageSize=10";
		document.dj_form.submit();
		}
</script>
<title>餐厅应付账款分析</title>
</head>
<body>
	<form  name="dj_form" method="post">
<div id="lable"><span>餐厅应付账款分析</span></div>
...
<div id="list-table">
	<table class="datas" >
		...
		<%
		int zj=0;
		int rows=rd.getTableRowsCount("dinnerYFZK");
		for(int a=0;a<rows;a++){%>
		<tr>
			<td><%=rd.getStringByDI("dinnerYFZK","pro",a) %>&gt;<%=rd.getStringByDI("dinnerYFZK","city",a) %></td>
			<td><%=rd.getStringByDI("dinnerYFZK","g_id",a) %></td>
			<td><%=rd.getStringByDI("dinnerYFZK","cmpny_name",a) %></td>
			<td><%=rd.getStringByDI("dinnerYFZK","persons",a) %></td>
			<td><%=rd.getStringByDI("dinnerYFZK","qdje",a) %></td>
		</tr>
		<%
		zj+=Integer.parseInt(rd.getStringByDI("dinnerYFZK","qdje",a));
		}
		%>
	</table>
</div>
...
<div id="page">
	...
	<span class="pagination-sum">
					...
					共<%=rd.getAttr("dinnerYFZK","rowsCount")==null?"0":rd.getAttr("dinnerYFZK","rowsCount") %> 条记录，
					每页 <%=pageSize%>条
	</span>
