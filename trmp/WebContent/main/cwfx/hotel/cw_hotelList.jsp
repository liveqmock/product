...
<%
	...
	int totalPage =1;
	int rowsCount = 1;
	if(rd.getTableRowsCount("hotelYFZK")>0){
	 ...
	 pageSize=Integer.parseInt((String)rd.getAttr("TA_GROUP_BASE", "pageSize"));
		totalPage=(Integer)rd.getAttr("hotelYFZK", "pagesCount");
		rowsCount = (Integer)rd.getAttr("hotelYFZK", "rowsCount");
	}
%>      
...
<script type="text/javascript">
	function findByLike(){
		document.dj_form.action="hotel_yfzk.?TA_GROUP_BASE@pageNO=1&TA_GROUP_BASE@pageSize=10";
...
<div id="lable"><span>酒店应付账款分析</span></div>
	...
	<table>
		<tr>
			...
			<td><select name="pro_id" id="pro0" USEDATA="dataSrc0" SUBCLASS="1"></select>
			  	...
			  	<select name="area_id" id="area0" USEDATA="dataSrc0" SUBCLASS="3"></select></td>
		</tr>
		<tr>
			<td>酒店名称：</td>
			<td><input name="hotelName"/></td>
...
<div id="list-table">
	<table class="datas" >
		...
		<%
		int zj=0;
		int rows=rd.getTableRowsCount("hotelYFZK");
		for(int a=0;a<rows;a++){%>
		<tr>
			<td><%=rd.getStringByDI("hotelYFZK","proName",a) %>&gt;<%=rd.getStringByDI("hotelYFZK","city",a) %>&gt;<%=rd.getStringByDI("hotelYFZK","area",a) %></td>
			<td><%=rd.getStringByDI("hotelYFZK","g_id",a) %></td>
			<td><%=rd.getStringByDI("hotelYFZK","hotel_name",a) %></td>
			<td><%=rd.getStringByDI("hotelYFZK","rooms",a) %></td>
			<td><%=rd.getStringByDI("hotelYFZK","pay_qd",a) %></td>
		</tr>
		<%
		zj+=Integer.parseInt(rd.getStringByDI("hotelYFZK","pay_qd",a));
		}
		%>
	</table>
</div>
...
<div id="page">
	...
	<span class="pagination-sum">
					...
					共<%=rd.getAttr("hotelYFZK","rowsCount")==null?"0":rd.getAttr("hotelYFZK","rowsCount") %> 条记录，
					每页 <%=pageSize%>条
	</span>
